...
export default function Edit() {
  ...
  const handleEdit = (periodo) => {
    // Redirigir a la página de edición de tabla detallada pasando el id del periodo
    navigate(`/EditTable?id=${periodo.id}&anio=${periodo.anio}&mes=${periodo.mes}`);
  };
      ...
      <div className={`page-container overflow-y-auto scroll-container ${menu ? "w-7/8" : "w-full"}`}>
        ...
        <div className="bg-header-bg p-10 rounded-[2.5rem] shadow-premium mb-12 relative">
          <div className="flex items-start justify-between mb-12">
            <div className="flex items-center gap-8">
              ...
              <div className="flex flex-col gap-1">
                <span className="text-sm font-bold text-white/40 tracking-[0.2em] uppercase">Gestión de datos</span>
                <h1 className="text-5xl font-display text-white tracking-tight uppercase">
                  Editar <span className="text-sidebar-bg/40 font-bold">Tablas</span>
                </h1>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl flex items-center px-8 py-5 w-full shadow-sm hover:shadow-md transition-shadow">
            ...
            <input
              type="text"
              placeholder="Buscar periodo para editar..."
              value={search}
        ...
        <div className="bg-white rounded-3xl shadow-premium border border-black/5 overflow-hidden">
          <table className="min-w-full divide-y divide-black/[0.03]">
            <thead className="bg-sidebar-bg">
              <tr>
                <th className="px-10 py-5 text-center text-xs font-black text-white/60 uppercase tracking-widest border-r border-white/5">Periodo / Registro</th>
                  ...
                  <tr key={t.id} className="hover:bg-main-bg/20 transition-colors group">
                    ...
                    <td className="px-10 py-6 text-center">
                      <button
                        ...
                        className="p-3 rounded-xl bg-gray-50 text-sidebar-bg/30 hover:text-accent hover:bg-accent/5 transition-all transform hover:scale-110 active:scale-95 shadow-sm hover:shadow-md"
