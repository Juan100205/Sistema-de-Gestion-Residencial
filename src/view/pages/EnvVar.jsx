...
import ExcelImporter from "../Components/EnvVar/ExcelImporter";
...
export default function EnvVar() {
    ...
    const loadPeriodoData = async () => {
      ...
      try {
        ...
        if (periodoObj) {
          ...
          // Buscar el periodo anterior directamente en la BD
          ...
          // Notificar brevemente si se usaron valores del mes anterior
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      // Fallback: If Agua Total/M3 Missing, use Previous Period
      let finalAguaRes = formData.agua_total_residencia;
      let finalAguaM3Res = formData.agua_m3_total_residencia;

      if ((!finalAguaRes || finalAguaRes == 0) || (!finalAguaM3Res || finalAguaM3Res == 0)) {
        // Find previous period
        const sortedPeriods = [...periodosRaw].sort((a, b) => {
          if (a.anio_end !== b.anio_end) return a.anio_end - b.anio_end;
          return a.mes_end - b.mes_end;
        });
        const currentIndex = sortedPeriods.findIndex(p => p.id === currentPeriodo.id);

        if (currentIndex > 0) {
          const prevPeriod = sortedPeriods[currentIndex - 1];
          if (!finalAguaRes || finalAguaRes == 0) {
            finalAguaRes = prevPeriod.agua_total_residencia;
            toast.info(`Usando "Factura Agua" del periodo anterior (${prevPeriod.mes_end}/${prevPeriod.anio_end}).`);
          }
          if (!finalAguaM3Res || finalAguaM3Res == 0) {
            finalAguaM3Res = prevPeriod.agua_m3_total_residencia;
            toast.info(`Usando "Consumo Agua Apts" del periodo anterior (${prevPeriod.mes_end}/${prevPeriod.anio_end}).`);
          }
        }
      }

      // Preparar datos con valores calculados (y fallbacks)
      const dataToSave = {
        ...formData,
      ...
      console.log("📤 EnvVar: Datos a guardar:", JSON.stringify(dataToSave, null, 2));
  ...
  const handleExcelCalculation = (calculatedValues) => {
    setFormData(prev => ({
      ...prev,
      ...calculatedValues
    }));
    toast.info("Valores calculados desde Excel aplicados. Revise y guarde.");
  };
  ...
  return (
    <div className="flex h-screen bg-sidebar-bg">
      ...
      <div className={`page-container overflow-y-auto scroll-container ${menu ? "w-7/8" : "w-full"}`}>
        ...
        {/* IMPORTADOR EXCEL (CALCULADORA DE LECTURAS) */}
        <ExcelImporter onDataCalculated={handleExcelCalculation} />
