...
export default function Delete() {
  ...
  const eliminar = async (periodo) => {
    if (!confirm(`¿Estás seguro de que deseas eliminar permanentemente el periodo ${periodo.name}? Esta acción no se puede deshacer y borrará todos los consumos asociados.`)) {
      return;
    }

    try {
      const result = await window.api.periodo.delete(periodo.id);
      if (result.success) {
        setTablas((prev) => prev.filter((t) => t.id !== periodo.id));
      } else {
        alert("Error al eliminar: " + result.error);
      }
    } catch (error) {
      console.error("Error en eliminar:", error);
      alert("Error inesperado al eliminar el periodo.");
    }
  };
      ...
      <div className={`page-container overflow-y-auto scroll-container ${menu ? "w-7/8" : "w-full"}`}>
        ...
        <div className="bg-header-bg p-10 rounded-[2.5rem] shadow-premium mb-12 relative">
            ...
            <div className="flex items-center gap-8">
              ...
              <div className="flex flex-col gap-1">
                ...
                <h1 className="text-5xl font-display text-white tracking-tight uppercase">
                  Eliminar <span className="text-sidebar-bg/40 font-bold">Tablas</span>
                </h1>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl flex items-center px-8 py-5 w-full shadow-sm hover:shadow-md transition-shadow">
            ...
            <input
              type="text"
              placeholder="Buscar periodo para eliminar..."
              value={search}
